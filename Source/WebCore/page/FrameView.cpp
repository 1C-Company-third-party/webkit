...
String FrameView::mediaType() const
{
    ...
    String overrideType = client ? client->overrideMediaType() : String();
...
bool FrameView::scrollToFragment(const URL& url)
{
    ...
    TextResourceDecoder* decoder = frame().document()->decoder();
    if (decoder && scrollToAnchor(decodeURLEscapeSequences(fragmentIdentifier, decoder->encoding())))
        return true;

    // Last chance: let's try to use UTF8 encoding
    return scrollToAnchor(decodeURLEscapeSequences(fragmentIdentifier, UTF8Encoding()));
}
