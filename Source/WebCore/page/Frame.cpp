...
Frame::Frame(Page& page, HTMLFrameOwnerElement* ownerElement, std::weak_ptr<FrameLoaderClient> frameLoaderClient)
...
Ref<Frame> Frame::create(Page* page, HTMLFrameOwnerElement* ownerElement, std::weak_ptr<FrameLoaderClient> client)
{
    ASSERT(page);
...
void Frame::setDocument(RefPtr<Document>&& newDocument)
{
    ...
    if (isMainFrame()) {
        ...
        if (!m_loader->documentLoader() && !m_loader->policyDocumentLoader() && !m_loader->isWebInspector())
        {
            auto client = m_loader->client().lock();
            if (client)
                client->dispatchDidChangeMainDocument();
        }
    }
