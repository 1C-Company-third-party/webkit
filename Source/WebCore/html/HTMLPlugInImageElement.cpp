...
bool HTMLPlugInImageElement::isImageType()
{
    ...
    {
        std::shared_ptr<FrameLoaderClient> client = frame->loader().client().lock();
        if (client)
            return client->objectContentType(document().completeURL(m_url), m_serviceType) == ObjectContentType::Image;
    }
...
bool HTMLPlugInImageElement::wouldLoadAsPlugIn(const String& url, const String& serviceType)
{
    ...
    std::shared_ptr<FrameLoaderClient> client = document().frame()->loader().client().lock();
    return client ? client->objectContentType(completedURL, serviceType) == ObjectContentType::PlugIn : false;
}
