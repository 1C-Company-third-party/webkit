...
void deleteAllCookies(const NetworkStorageSession& session)
{
    ...
    // Let's simply delete cookies file.
    String cookieJar = CurlContext::singleton().getCookieJarFileName();
    DeleteFileW(cookieJar.charactersWithNullTermination().data());

    // And try to clear cookies from memory.
    CURL* curl = curl_easy_init();
    if (!curl)
        return;

    auto& shareHandle = CurlContext::singleton().shareHandle();

    curl_easy_setopt(curl, CURLOPT_SHARE, shareHandle.handle());
