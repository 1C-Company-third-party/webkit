...
void FontPlatformData::platformDataInit(HFONT font, float size, HDC hdc, WCHAR* faceName, bool synthItalic)
{
    cairo_font_face_t* fontFace = nullptr;
    if (synthItalic)
    {
        String face(faceName);
        fontFace = cairo_toy_font_face_create(face.ascii().data(), CAIRO_FONT_SLANT_ITALIC, CAIRO_FONT_WEIGHT_NORMAL);
    }
    else
    { 
        fontFace = cairo_win32_font_face_create_for_hfont(font);
    }
