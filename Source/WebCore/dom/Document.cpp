...
void Document::resume(ActiveDOMObject::ReasonForSuspension reason)
{
    ...
    std::shared_ptr<FrameLoaderClient> client = m_frame->loader().client().lock();
    if (client)
        client->dispatchDidBecomeFrameset(isFrameSet());
...
void Document::initSecurityContext()
{
    ...
    if (client)
    {
        String overrideContentSecurityPolicy = client->overrideContentSecurityPolicy();
    ...
    if (!settings().webSecurityEnabled()) {
        ...
        if (settings().allowUniversalAccessFromFileURLs() || (client && client->shouldForceUniversalAccessFromLocalURL(m_url))) {
