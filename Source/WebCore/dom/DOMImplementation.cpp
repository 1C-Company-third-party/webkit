...
Ref<Document> DOMImplementation::createDocument(const String& type, Frame* frame, const URL& url)
{
    ...
    if (type != "text/plain")
    {
        if (pluginData && pluginData->supportsWebVisibleMimeType(type, allowedPluginTypes))
            return PluginDocument::create(frame, url);

        if (frame)
        {
            std::shared_ptr<FrameLoaderClient> loaderClient = frame->loader().client().lock();
            if (loaderClient && loaderClient->shouldAlwaysUsePluginDocument(type))
                return PluginDocument::create(frame, url);
        }
    }
